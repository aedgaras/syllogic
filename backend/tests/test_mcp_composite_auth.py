...
class TestPfKeyRouting:
    ...
    async def test_pf_prefix_routes_to_api_key_provider(self, provider):
        ...
        with (
            ...
            patch.object(provider.jwt, "verify_token", AsyncMock()) as m_jwt,
        ):
    ...
    async def test_invalid_pf_returns_none(self, provider):
        with (
            patch.object(provider.api_key, "verify_token", AsyncMock(return_value=None)),
            patch.object(provider.jwt, "verify_token", AsyncMock()) as m_jwt,
        ):
...
class TestJwtRouting:
    ...
    async def test_non_pf_routes_to_jwt(self, provider):
        ...
        with (
            patch.object(provider.api_key, "verify_token", AsyncMock()) as m_api,
            patch.object(provider.jwt, "verify_token", AsyncMock(return_value=fake_token)),
        ):
    ...
    async def test_jwt_user_id_normalized_from_sub(self, provider):
        ...
        with patch.object(provider.jwt, "verify_token", AsyncMock(return_value=fake_token)):
            result = await provider.verify_token("eyJhbGciOi...")
    ...
    async def test_invalid_jwt_returns_none(self, provider):
        with patch.object(provider.jwt, "verify_token", AsyncMock(return_value=None)):
