...
def test_daily_next_run_today_if_time_not_passed():
    after = datetime(2026, 7, 19, 6, 0, tzinfo=timezone.utc)  # 06:00 UTC
    result = compute_next_run_at("DAILY", time(8, 0), "UTC", None, None, after)
    assert result == datetime(2026, 7, 19, 8, 0, tzinfo=timezone.utc)


def test_daily_next_run_tomorrow_if_time_passed():
    after = datetime(2026, 7, 19, 9, 0, tzinfo=timezone.utc)  # after 08:00
    result = compute_next_run_at("DAILY", time(8, 0), "UTC", None, None, after)
    assert result == datetime(2026, 7, 20, 8, 0, tzinfo=timezone.utc)


def test_weekly_picks_next_matching_weekday():
    # 2026-07-19 is a Sunday (weekday()==6). Target Monday (0).
    after = datetime(2026, 7, 19, 6, 0, tzinfo=timezone.utc)
    result = compute_next_run_at("WEEKLY", time(8, 0), "UTC", 0, None, after)
    assert result == datetime(2026, 7, 20, 8, 0, tzinfo=timezone.utc)  # Monday 2026-07-20
...
def test_monthly_picks_day_of_month_next_month_if_passed():
    ...
    assert result == datetime(2026, 8, 1, 8, 0, tzinfo=timezone.utc)
...
def test_timezone_converts_local_time_to_utc():
    ...
    after = datetime(2026, 7, 19, 0, 0, tzinfo=timezone.utc)  # UTC
    result = compute_next_run_at("DAILY", time(8, 0), "Europe/Brussels", None, None, after)
    assert result == datetime(2026, 7, 19, 6, 0, tzinfo=timezone.utc)
