...
class RevolutCSVAdapter(BankAdapter):
    ...
    def _detect_delimiter(self) -> str:
        ...
        sniffer = csv.Sniffer()
        sample = (
            csv_content_normalized[:2048]
            if len(csv_content_normalized) > 2048
            else csv_content_normalized
        )
        try:
    ...
    def _parse_transaction_row(
        ...
        for key in row.keys():
            ...
            if "completed" in key_lower and "date" in key_lower:
                ...
                key_lower == "date" or "transaction date" in key_lower or "booked date" in key_lower
            ):
                date_str = row.get(key)
                break
        ...
        if not date_str:
            date_str = (
                ...
                or row.get("Booked Date")
                or row.get("completed_date")
                or row.get("started_date")
            )
