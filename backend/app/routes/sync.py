...
def sync_plaid(
    ...
    try:
        ...
        adapter = PlaidAdapter(
            access_token=access_token, client_id=client_id, secret=secret, environment=environment
        )
        ...
        raise HTTPException(
            status_code=500, detail="Plaid library not installed. Run: pip install plaid-python"
        )
    except Exception as e:
        import traceback
...
def get_sync_status(account_id: str, user_id: Optional[str] = None, db: Session = Depends(get_db)):
