...
def enqueue_csv_import(request: EnqueueImportRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        csv_import = (
            db.query(CsvImport)
            .filter(CsvImport.id == request.csv_import_id, CsvImport.user_id == user_id)
            .first()
        )
        ...
        if request.daily_balances:
            daily_balances_data = [
                {"date": bal.date, "balance": bal.balance} for bal in request.daily_balances
            ]
        ...
        logger.info(f"Enqueued CSV import {request.csv_import_id} as Celery task {task.id}")
...
def get_import_status(import_id: str, user_id: Optional[str] = None, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        csv_import = (
            db.query(CsvImport)
            .filter(CsvImport.id == import_id, CsvImport.user_id == resolved_user_id)
            .first()
        )
