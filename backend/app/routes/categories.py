...
def list_categories(user_id: Optional[str] = None, db: Session = Depends(get_db)):
...
def get_category(category_id: UUID, user_id: Optional[str] = None, db: Session = Depends(get_db)):
...
def categorize_transactions_batch(
    request: BatchCategorizeRequest, db: Session = Depends(get_db), user_id: Optional[str] = None
):
    """
    ...
    try:
        logger.info(
            f"[CATEGORIZE] Batch categorizing {len(request.transactions)} transactions (use_llm={request.use_llm})"
        )
        ...
        logger.info(
            f"[CATEGORIZE] Phase 1 complete. Deterministic matches: {deterministic_count}, Unmatched: {len(unmatched_indices)}"
        )
        ...
        if request.use_llm and unmatched_indices:
            ...
            if not client:
                ...
                logger.warning(
                    "[CATEGORIZE] LLM client not available - skipping LLM categorization"
                )
            else:
                # Prepare batch for LLM
                llm_batch = []
                logger.info(
                    f"[CATEGORIZE] Preparing LLM batch for {len(unmatched_indices)} transactions"
                )
                for idx in unmatched_indices:
                    txn = request.transactions[idx]
                    logger.info(
                        f"[CATEGORIZE] Transaction {idx}: transaction_type='{txn.transaction_type}', amount={txn.amount}, description='{txn.description[:50]}...'"
                    )
                    llm_batch.append(
                        {
                            "index": idx,
                ...
                logger.info(
                    f"[CATEGORIZE] LLM batch prepared with {len(llm_batch)} items. First item keys: {list(llm_batch[0].keys()) if llm_batch else 'empty'}"
                )

                try:
                    llm_results, total_tokens, total_cost = matcher.match_categories_batch_llm(
                        llm_batch
                    )
                    ...
                    if len(llm_results) < len(unmatched_indices):
                        ...
                        llm_warnings.append(
                            f"LLM was unable to categorize {missing} out of {len(unmatched_indices)} transactions."
                        )
                except Exception as e:
        ...
        logger.info(
            f"[CATEGORIZE] Batch complete. Categorized: {categorized_count}/{len(request.transactions)}"
        )
...
def _parse_csv_content(csv_content: str) -> List[TransactionInput]:
    ...
    if len(lines) < 2:
        raise HTTPException(
            status_code=400, detail="CSV must have a header row and at least one data row"
        )
    ...
    for i, line in enumerate(lines[1:], start=2):
        ...
        try:
            ...
            description = (
                values[desc_idx].strip()
                if desc_idx >= 0 and desc_idx < len(values) and values[desc_idx]
                else None
            )
            merchant = (
                values[merchant_idx].strip()
                if merchant_idx >= 0 and merchant_idx < len(values) and values[merchant_idx]
                else None
            )
...
async def categorize_transactions_batch_csv(
    file: UploadFile = File(
        ...,
        description="CSV file with transactions. Required columns: amount. Optional columns: description, merchant",
    ),
    use_llm: bool = Form(
        True, description="Whether to use LLM fallback for unmatched transactions"
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="CSV file encoding not supported. Please use UTF-8 encoding.",
                )

        # Parse CSV
        transactions = _parse_csv_content(csv_content)
        logger.info(
            f"[CATEGORIZE CSV] Parsed {len(transactions)} transactions from CSV file: {file.filename}"
        )

        # Create batch request and call the batch categorization logic
        batch_request = BatchCategorizeRequest(transactions=transactions, use_llm=use_llm)
