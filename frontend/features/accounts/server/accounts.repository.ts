...
export async function updateOwnedAccount(accountId: string, input: UpdateAccountInput) {
  ...
  if (input.startingBalance !== undefined) values.startingBalance = String(input.startingBalance);
...
export async function permanentlyDeleteAccount(userId: string, accountId: string) {
  await db.delete(recurringTransactions).where(and(
    eq(recurringTransactions.accountId, accountId),
    eq(recurringTransactions.userId, userId),
  ));
  await db.delete(subscriptionSuggestions).where(and(
    eq(subscriptionSuggestions.accountId, accountId),
    eq(subscriptionSuggestions.userId, userId),
  ));
  const deletedBalances = await db.delete(accountBalances)
    .where(eq(accountBalances.accountId, accountId))
    .returning({ id: accountBalances.id });
  const deletedTransactions = await db.delete(transactions).where(and(
...
export async function getTransactionSum(accountId: string, through?: Date) {
  const where = through
    ? and(eq(transactions.accountId, accountId), lte(transactions.bookedAt, through))
    : eq(transactions.accountId, accountId);
...
export function getStoredBalanceHistory(accountId: string, startDate: Date | null) {
  return db.query.accountBalances.findMany({
    where: startDate
      ? and(eq(accountBalances.accountId, accountId), gte(accountBalances.date, startDate))
      : eq(accountBalances.accountId, accountId),
    orderBy: [desc(accountBalances.date)],
  });
}

export async function getDailyTransactionChanges(accountId: string, startDate: Date) {
  return db.select({
    date: sql<string>`DATE(${transactions.bookedAt})`,
    amount: sql<string>`SUM(${transactions.amount})`,
  }).from(transactions)
    .where(and(eq(transactions.accountId, accountId), gte(transactions.bookedAt, startDate)))
...
export function getLatestStoredBalance(accountId: string, date: Date) {
  return db.query.accountBalances.findFirst({
    where: and(eq(accountBalances.accountId, accountId), lte(accountBalances.date, date)),
    orderBy: [desc(accountBalances.date)],
  });
}
