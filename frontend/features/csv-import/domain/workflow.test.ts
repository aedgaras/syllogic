...
import { createImportWorkflow, csvImportWorkflowReducer, sanitizeMappingForContext } from "./workflow";
import type { ColumnMapping } from "./contracts";

const mapping: ColumnMapping = {
  date: "Date", amount: "Amount", description: "Description", merchant: "Merchant",
  transactionType: "Type", fee: null, state: null, startingBalance: null, endingBalance: null,
};
...
describe("CSV import workflow", () => {
  it("advances through upload, mapping, preview, and completion", () => {
    ...
    state = csvImportWorkflowReducer(state, { type: "UPLOAD_COMPLETED", importId: "import-1" });
  ...
  it("keeps mapping differences in the context policy", () => {
    expect(sanitizeMappingForContext(mapping, "dashboard")).toMatchObject({ merchant: null, transactionType: null });
