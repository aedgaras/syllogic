...
export function usePreviewController(context: ImportContext) {
  ...
  const [balanceVerification, setBalanceVerification] =
    useState<BalanceVerification | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!importSession.columnMapping) {
        toast.error("Column mapping not found");
        router.push(policy.mappingPath(importId));
        return;
      }
  ...
  const groups = useMemo(
    () => ({
      toImport: transactions.filter((tx) =>
        selectedIndices.includes(tx.rowIndex),
      ),
      skipped: transactions.filter(
        (tx) => !selectedIndices.includes(tx.rowIndex),
      ),
    }),
  ...
  return {
    ...
    ...groups,
    enqueue,
    goBack: () => importId && router.push(policy.mappingPath(importId)),
  };
}
