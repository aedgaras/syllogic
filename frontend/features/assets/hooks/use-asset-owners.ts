...
export function useAssetOwners(type: AssetType, id: string | undefined, ownerIds: string[], people: AssetPerson[]) {
  return useQuery({
    queryKey: assetQueryKeys.owners(type, id ?? "idle"),
    enabled: Boolean(id),
    queryFn: async (): Promise<AssetOwner[]> => {
      try { return await loadAssetOwners(type, id!); }
      catch {
        if (ownerIds.length) return ownerIds.map((personId) => ({ personId, share: null }));
