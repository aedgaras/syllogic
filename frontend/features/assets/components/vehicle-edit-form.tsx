...
export function VehicleEditForm({
  ...
  onSubmit: (values: VehicleEditValues) => Promise<void>;
}) {
  const {
    control,
    register,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{translate("editVehicle")}</DialogTitle>
          <DialogDescription>
            {translate("updateYourVehicleDetails")}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="grid gap-4 py-4">
            ...
            >
              <Input
                aria-label={translate("vehicleName")}
                {...register("name")}
              />
            </Field>
            <Controller
              ...
              name="vehicleType"
              render={({ field }) => (
                <Field label={translate("vehicleType")}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label={translate("makeOptional")}>
                <Input
                  aria-label={translate("makeOptional")}
                  {...register("make")}
                />
              </Field>
              <Field label={translate("modelOptional")}>
                <Input
                  aria-label={translate("modelOptional")}
                  {...register("model")}
                />
              </Field>
            </div>
            <Field
              label={translate("yearOptional")}
              error={errors.year?.message}
            >
              <Input
                aria-label={translate("yearOptional")}
                type="number"
                {...register("year")}
              />
            </Field>
