...
describe("asset view-model mappers", () => {
  it("projects persistence rows into narrow asset contracts", () => {
    const account = toAccountAssetViewModel(
      {
        ...
        institution: null,
        currency: null,
        functionalBalance: null,
        logo: null,
      } as never,
      ["p1"],
    );
    const property = toPropertyAssetViewModel({
      id: "p1",
      name: "Home",
    ...
    const vehicle = toVehicleAssetViewModel({
      id: "v1",
      name: "Car",
      vehicleType: "car",
      make: null,
    ...
    expect(property).toEqual(
      expect.objectContaining({ id: "p1", value: "10" }),
    );
