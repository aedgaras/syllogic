...
export function calculateStringSimilarity(left: string, right: string): number {
  ...
  return Math.max(
    0,
    ((maxLength - levenshteinDistance(a, b)) / maxLength) * 100,
  );
}
...
export function filterGapOutliers(values: number[], multiplier = 2): number[] {
  ...
  return values.filter(
    (value) => Math.abs(value - mean) <= multiplier * deviation,
  );
}

export function detectFrequencyFromGaps(gaps: number[]): {
  frequency: SubscriptionFrequency | null;
  confidence: number;
} {
  const filtered = filterGapOutliers(
    gaps.filter((gap) => Number.isFinite(gap) && gap > 0),
  );
  if (filtered.length === 0) return { frequency: null, confidence: 0 };

  const average = filtered.reduce((sum, gap) => sum + gap, 0) / filtered.length;
  let best: { frequency: SubscriptionFrequency | null; score: number } = {
    frequency: null,
    score: 0,
  };
  for (const [frequency, range] of Object.entries(frequencyRanges) as Array<
    [SubscriptionFrequency, { min: number; max: number; target: number }]
  >) {
    if (average < range.min || average > range.max) continue;
    const score =
      1 - Math.abs(average - range.target) / ((range.max - range.min) / 2);
  ...
  return {
    ...
    confidence: Math.round((best.score + consistency) * 50),
  };
}
...
export function scoreSubscriptionMatch(
  target: MatchTarget,
  ...
  } else if (difference <= targetAmount * 0.05) {
    score += 20;
  ...
  if (
    ...
    (candidate.categoryId === target.categoryId ||
      candidate.categorySystemId === target.categoryId)
  ) {
    score += 10;
  ...
  if (
    !target.merchant &&
    !candidate.merchant &&
    calculateStringSimilarity(target.name, candidate.description ?? "") >= 70
  ) {
    score += 20;
