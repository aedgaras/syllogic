...
import { mapTransactionListRows, type TransactionListRow } from "./transaction-list.mapper";
...
function resolveSortOrder(sort: TransactionSortField, order: TransactionSortOrder) {
  if (sort === "amount") return order === "asc" ? [asc(transactions.amount), desc(transactions.bookedAt)] : [desc(transactions.amount), desc(transactions.bookedAt)];
  if (sort === "description") return order === "asc" ? [asc(transactions.description), desc(transactions.bookedAt)] : [desc(transactions.description), desc(transactions.bookedAt)];
  if (sort === "merchant") return order === "asc" ? [asc(transactions.merchant), desc(transactions.bookedAt)] : [desc(transactions.merchant), desc(transactions.bookedAt)];
  return order === "asc" ? [asc(transactions.bookedAt), desc(transactions.id)] : [desc(transactions.bookedAt), desc(transactions.id)];
}

export async function hydrateResolvedAccountLogos(rows: TransactionListRow[]) {
  const uniqueAccounts = Array.from(new Map(rows.filter((row) => row.account).map((row) => [
    row.account!.id,
    { id: row.account!.id, institution: row.account!.institution, logoId: row.account!.logoId, logo: row.account!.logo },
  ])).values());
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      account: {
        ...
        logo: resolved.logo ? { id: resolved.logo.id, logoUrl: resolved.logo.logoUrl, updatedAt: resolved.logo.updatedAt ?? null } : null,
      },
    };
  });
}

async function getLatestTransactionDateForScope(userId: string, accountIds: string[]) {
  const conditions = [eq(transactions.userId, userId)];
  if (accountIds.length) conditions.push(inArray(transactions.accountId, accountIds));
  const result = await db.select({ latestDate: sql<string>`MAX(${transactions.bookedAt})` }).from(transactions).where(and(...conditions));
...
async function buildWhereClause(userId: string, input: TransactionsQueryState) {
  ...
  if (input.from) {
    ...
    resolvedTo = input.to ? new Date(`${input.to}T23:59:59.999Z`) : await getLatestTransactionDateForScope(userId, accountIds);
  ...
  if (input.search) conditions.push(or(ilike(transactions.description, `%${input.search}%`), ilike(transactions.merchant, `%${input.search}%`))!);
  ...
  if (categoryIds.length || includeUncategorized) {
    ...
    if (categoryIds.length) {
      ...
      categoryConditions.push(and(isNull(transactions.categoryId), inArray(transactions.categorySystemId, categoryIds))!);
    }
    if (includeUncategorized) categoryConditions.push(and(isNull(transactions.categoryId), isNull(transactions.categorySystemId))!);
    conditions.push(or(...categoryConditions)!);
  }
  const includesPending = input.status.includes("pending");
  const includesCompleted = input.status.includes("completed");
  if (includesPending !== includesCompleted) conditions.push(includesPending ? eq(transactions.pending, true) : or(eq(transactions.pending, false), isNull(transactions.pending))!);
  ...
  if (subscriptionIds.length || includesNoSubscription) {
    ...
    if (subscriptionIds.length) subscriptionConditions.push(inArray(transactions.recurringTransactionId, subscriptionIds));
    if (includesNoSubscription) subscriptionConditions.push(isNull(transactions.recurringTransactionId));
  ...
  if (includesAnalytics !== includesNonAnalytics) conditions.push(eq(transactions.includeInAnalytics, includesAnalytics));
  const minAmount = toNumberOrUndefined(input.minAmount);
  const maxAmount = toNumberOrUndefined(input.maxAmount);
  if (minAmount !== undefined) conditions.push(sql`ABS(${transactions.amount}) >= ${minAmount}`);
  if (maxAmount !== undefined) conditions.push(sql`ABS(${transactions.amount}) <= ${maxAmount}`);

  return { whereClause: and(...conditions)!, resolvedFrom, resolvedTo, effectiveHorizon };
}

export const transactionListRepository: TransactionListRepository = {
  async getPage(userId, input) {
    const { whereClause, resolvedFrom, resolvedTo, effectiveHorizon } = await buildWhereClause(userId, input);
    ...
    const [countRows, rawRows, totalsRows] = await Promise.all([
      ...
      db.query.transactions.findMany({
        ...
        with: {
          account: { with: { logo: { columns: { id: true, logoUrl: true, updatedAt: true } } } },
          category: true,
          categorySystem: true,
    ...
    const rows = await hydrateResolvedAccountLogos(rawRows as TransactionListRow[]);
    return {
      rows: mapTransactionListRows(rows, (row) => console.warn("[getTransactionPage] Transaction missing account relation", { transactionId: row.id, accountId: row.accountId })),
