...
import {
  getTransactionPage,
  type TransactionListRepository,
} from "./get-transaction-page";
...
describe("getTransactionPage", () => {
  ...
  it("delegates an authenticated query to the repository", async () => {
    ...
    const repository: TransactionListRepository = {
      getPage: vi.fn().mockResolvedValue(page),
    };
    await expect(getTransactionPage("user-1", query, repository)).resolves.toBe(
      page,
    );
