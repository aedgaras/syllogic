...
import {
  ...
  skipNextOccurrence,
} from "@/lib/actions/recurring-schedule.gateway";
...
export interface CreateSubscriptionFromTransactionInput {
  ...
  /** Book future occurrences automatically instead of waiting for the bank. */
  autoGenerate?: boolean;
  /** Date of the first occurrence to book; required when autoGenerate is on. */
...
export async function createSubscriptionFromTransaction(
  ...
  try {
    ...
    if (input.autoGenerate && !input.nextDueDate) {
      return {
        success: false,
        error: "A next due date is required to generate transactions automatically",
      };
    }
    ...
    if (
      ...
      return { success: false, error: "End date must be after the next due date" };
    }
    ...
    const [created] = await db
      ...
      .values({
        ...
        frequency: input.frequency,
        description: null,
        autoGenerate: Boolean(input.autoGenerate),
        nextDueDate: input.autoGenerate ? input.nextDueDate : null,
        endDate: input.autoGenerate ? input.endDate || null : null,
      })
      .returning({ id: recurringTransactions.id });
...
/**
 * Materialize the next scheduled occurrence of a recurring entry right now,
 * instead of waiting for the daily job to reach its due date.
 */
export async function generateRecurringTransactionNow(
  subscriptionId: string,
): Promise<{
  success: boolean;
  error?: string;
  ...
  try {
    const result = await generateNextOccurrence(subscriptionId, userId);
    revalidatePath("/transactions");
    revalidatePath("/subscriptions");
    ...
    logger.error("Failed to generate recurring transaction", { error });
    return {
      success: false,
      error:
...
/**
 * Advance a recurring entry past its next occurrence without booking it —
 * the month a subscription is paused or a bill is waived.
 */
export async function skipRecurringTransactionOccurrence(
  subscriptionId: string,
): Promise<{
  success: boolean;
  error?: string;
  ...
  try {
    const result = await skipNextOccurrence(subscriptionId, userId);
    revalidatePath("/subscriptions");
    return {
      success: true,
      skippedDate: result.skippedDate,
      nextDueDate: result.nextDueDate,
      autoGenerate: result.autoGenerate,
    };
  } catch (error) {
    logger.error("Failed to skip recurring occurrence", { error });
    return {
      success: false,
      error:
