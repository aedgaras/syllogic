...
describe("LocalStorageProvider path containment", () => {
  it("stores and reads files inside the configured root", async () => {
    ...
    await expect(storage.download("profile/user.webp")).resolves.toEqual(
      Buffer.from("image"),
    );
  });
  ...
  it.each(["../escape", "nested/../../escape", "/tmp/escape"])(
    ...
    async (unsafePath) => {
      ...
      await expect(
        storage.upload(unsafePath, Buffer.from("no")),
      ).rejects.toThrow(/storage root|relative path/);
      await expect(storage.download(unsafePath)).rejects.toThrow(
        /storage root|relative path/,
      );
      await expect(storage.delete(unsafePath)).rejects.toThrow(
        /storage root|relative path/,
      );
    },
  );
