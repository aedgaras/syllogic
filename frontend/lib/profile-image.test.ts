...
describe("normalizeProfileImage", () => {
  it("rejects SVG even when presented as an image", async () => {
    ...
    await expect(normalizeProfileImage(file)).rejects.toThrow(/supported image/);
  });
  ...
  it("decodes and re-encodes raster images as WebP", async () => {
    ...
    const output = await normalizeProfileImage(new File([png], "avatar.svg", { type: "image/svg+xml" }));
