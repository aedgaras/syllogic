...
vi.mock("@/components/transactions/csv-mapping-table", () => ({
  CsvMappingTable: ({
    ...
    onMappingChange: (value: Record<string, unknown>) => void;
  }) => (
    <button
      onClick={() =>
        onMappingChange({
...
vi.mock("@/components/transactions/csv-preview-table", () => ({
  CsvPreviewTable: ({
    ...
    transactions: Array<{ rowIndex: number }>;
  }) => (
    <div data-testid="preview-rows">
      {transactions.map((item) => item.rowIndex).join(",")}
    </div>
  ),
}));
...
describe("dashboard CSV import characterization", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.getCsvImportSession.mockResolvedValue({
      id: "import-1",
    ...
    mocks.enqueueBackgroundImport.mockResolvedValue({
      success: true,
      importId: "job-1",
      totalTransactions: 1,
    });
  ...
  it("sanitizes unsupported mappings before saving and opens preview", async () => {
    render(<MappingPage />);
    fireEvent.click(
      await screen.findByRole("button", { name: "change optional mapping" }),
    );
    fireEvent.click(
    ...
    expect(mocks.saveColumnMapping.mock.calls[0][1]).toMatchObject({
      merchant: null,
      transactionType: null,
    });
    expect(mocks.push).toHaveBeenCalledWith(
      "/transactions/import/preview?id=import-1",
    );
  });
  ...
  it("preselects non-duplicates and enqueues only those rows", async () => {
    ...
    await waitFor(() =>
      expect(mocks.enqueueBackgroundImport).toHaveBeenCalledWith(
        "import-1",
        [0],
      ),
    );
