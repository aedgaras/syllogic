...
vi.mock("@tanstack/react-query", () => ({
  ...
  useQuery: ({ queryKey }: { queryKey: string[] }) => ({
    data:
      queryKey[1] === "logo-lookup-enabled"
        ? false
        : [{ personId: "person-1", share: null }],
    isSuccess: true,
  }),
  useMutation: ({
    mutationFn,
...
vi.mock("@/lib/actions/logos", () => ({
  hasLogoApiKey: vi.fn().mockResolvedValue(false),
  searchLogo: vi.fn(),
}));
vi.mock("@/lib/people/client", () => ({
  fetchOwners: vi
    .fn()
    .mockResolvedValue([{ personId: "person-1", share: null }]),
  ownersQueryKey: (type: string, id: string) => ["owners", type, id],
  saveOwners: mocks.saveOwners,
}));
vi.mock("@/components/command-palette-context", () => ({
  useRegisterCommandPaletteCallbacks: vi.fn(),
}));
vi.mock("@/components/assets/add-asset-dialog", () => ({
  AddAssetDialog: () => null,
}));
vi.mock("@/components/accounts/update-balance-dialog", () => ({
  UpdateBalanceDialog: () => null,
}));
vi.mock("@/components/household/owner-badges", () => ({
  OwnerBadges: () => null,
}));
vi.mock("@/components/household/owners-field", () => ({
  OwnersField: () => null,
}));
...
describe("AssetManagement account workflow characterization", () => {
  ...
  it("hydrates the edit form, saves account fields and ownership, then refreshes", async () => {
    ...
    expect(mocks.saveOwners).toHaveBeenCalledWith("account", "account-1", [
      { personId: "person-1", share: null },
    ]);
