...
function formatReportSchedule(report: Report) {
  const nextRun = report.next_run_at ? new Date(report.next_run_at).toLocaleString() : "-";
  ...
  return (
    <ul className="divide-y divide-border border border-border">
      {reports.map((report) => {
        ...
        return (
          <li key={report.id} className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0 space-y-1">
              <Link href={`/reports/${report.id}`} className="break-words font-medium text-foreground hover:underline">
                {report.name}
              </Link>
